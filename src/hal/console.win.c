...
read_line(string_t * recycle_buffer)
{
  ...
  if(recycle_buffer->length == 0) {
    ...
    if(recycle_buffer->buffer == nullptr) return EM_RESULT_OUT_OF_MEMORY;
    recycle_buffer->buffer[0] = '\0';
  }
  if(recycle_buffer->buffer == nullptr) return EM_RESULT_INVALID_ARGUMENT;
  DWORD    readed = 0;
  ...
  while(1) {
    ...
    CHKERR(em_reallocarray(
      &newBuf, recycle_buffer->buffer, sizeof(char_t), recycle_buffer->length + ADD_SIZE));
  ...
  CHKERR(
    em_reallocarray(&(recycle_buffer->buffer), recycle_buffer->buffer, sizeof(char_t), readed + 1));
