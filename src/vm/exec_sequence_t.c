...
go_topological_sort(string_t * dependency, node_or_tuple_t * nt) {
  ...
  switch(nt->kind) {
  case NODE_OR_TUPLE_NONE: return false;
...
em_result
topological_sort(machine_t * machine, list_t /*<exec_sequence_t>*/ ** es, list_t /*<exec_sequence_t>*/ *** last_es) {
  em_result errres = EM_RESULT_OK;
  ...
  for(list_t * li = empty.head; li != nullptr; li = LIST_NEXT(li)) {
	...
	if((li_v_v->node_definition != nullptr && string_compare(&(li_v_v->node_definition->name), (string_t *)((*refs)->value)))
