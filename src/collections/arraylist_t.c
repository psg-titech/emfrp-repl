...
arraylist_resize(arraylist_t * self, size_t item_size)
{
  ...
  if(new_buf == nullptr) return EM_RESULT_OUT_OF_MEMORY;
  self->buffer   = new_buf;
...
arraylist_append(arraylist_t * self, size_t item_size, void * value)
{
  ...
  em_result errres;
  if(self->length + 1 > self->capacity) CHKERR(arraylist_resize(self, item_size));
...
arraylist_insert(arraylist_t * self, size_t index, size_t item_size, void * value)
{
  ...
  em_result errres;
#if DEBUG
  if(index > self->length || index < 0) return EM_RESULT_OUT_OF_INDEX;
  if(index == self->length) return arraylist_append(self, item_size, value);
#endif
  if(self->length + 1 > self->capacity) CHKERR(arraylist_resize(self, item_size));
  memmove(
    (void *)((size_t)self->buffer + (item_size * (index + 1))),
    (void *)((size_t)self->buffer + (item_size * index)), (self->length - index) * item_size);
...
arraylist_remove(arraylist_t * self, void * out, size_t index, size_t item_size)
{
  ...
  memcpy(
    (void *)((size_t)self->buffer + (item_size * index)),
...
void
arraylist_removeinsert(
  arraylist_t * self, void ** out, size_t remove_index, size_t insert_index, size_t item_size,
  void * value)
{
  item_size /= sizeof(char);
  if(item_size > 0 && out != nullptr)
  ...
  if(remove_index == insert_index) {
    ...
    memcpy(
      (void *)((size_t)self->buffer + (item_size * remove_index)),
      (void *)((size_t)self->buffer + (item_size * remove_index) + item_size),
      item_size * (insert_index - remove_index));
  else
    memmove(
      (void *)((size_t)self->buffer + (item_size * (insert_index + 1))),
