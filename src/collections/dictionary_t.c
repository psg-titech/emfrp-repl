...
em_result
dictionary_add(dictionary_t * out, void * value, size_t value_size, size_t(hasher(void *)), bool(comparer(void *, void *)), void(replacer(void *, void *)), void * replacer_arg) {
  em_result errres;
  size_t hashed = hasher(value) % DICTIONARY_TABLE_SIZE;
  void * overwrite;
  if(list_search(out->values[hashed], &overwrite, comparer, value)) {
    if(replacer != nullptr)
      replacer(overwrite, replacer_arg);
    else
...
em_result
dictionary_add2(dictionary_t * out, void * value, size_t value_size, size_t(hasher(void *)), bool(comparer(void *, void *)), void(replacer(void *, void *)), void * replacer_arg, void ** entry_ptr) {
  em_result errres;
  size_t hashed = hasher(value) % DICTIONARY_TABLE_SIZE;
  void * overwrite;
  if(list_search(out->values[hashed], &overwrite, comparer, value)) {
