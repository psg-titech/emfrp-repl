 ...
 */
  em_result
  machine_add_node_ast(machine_t * self, exec_sequence_t ** out, struct parser_node_t * n);
  ...
  machine_restore_stack_state(machine_t * self, stack_state_t state)
  {
    ...
    if(self->memory_manager->state == MEMORY_MANAGER_STATE_MARK) {
      for(int i = state; i < self->stack->value.stack.length; ++i)
        CHKERR(memory_manager_push_worklist_uncheck_state(
          self->memory_manager, self->stack->value.stack.data[i]));
    }
 ...
 */
  ...
  machine_pop_variable_table(machine_t * self)
  {
    ...
    if(self->variable_table != nullptr) self->variable_table = self->variable_table->parent;
    return EM_RESULT_OK;
  }
 ...
 * \return The result
 */
  em_result machine_match(
    machine_t * self, list_t /*<deconstructor_t>*/ * nt, struct object_t ** vs, int length);
 ...
 * \return The result, if the matching is succeeded, it returns true.
 */
  bool machine_test_matches(machine_t * self, deconstructor_t * deconst, struct object_t * v);
